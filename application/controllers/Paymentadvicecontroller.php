...
class PaymentAdviceController extends CI_Controller
{
    ...
    public function paymentadviceh()
    {
        $adviceHistory = $this->paymentadvicemodel->getPaymentAdvices();
        JSONResponse($adviceHistory);
    }
    ...
    public function updatePaymentAdvice()
    {
        ...
        if(isset($data))
        {
            $advice = $this->paymentadvicemodel->getAdvice($data['adviceCode']);

            if($advice->payment_type === 'One Location')
            {
                $newAmount = str_replace(",", "", $data['amountPaid']);
                ...
                [
                    ...
                    'old_amount'        => $advice->amount_paid,
                    'new_amount'        => $newAmount
                ];
                $this->db->trans_start();
                $this->db->insert('payment_advice_edit_history', $history);

                $this->db->WHERE('id', $advice->id)
                         ->UPDATE('payment_advice', ['amount_paid' => $newAmount]);
                $this->db->trans_complete();
            }
            else if($advice->payment_type === 'Multi Location')
            {
                $newAmount = str_replace(",", "", $data['totalAmountPaid']);
                ...
                for($x=0;$x < count($data['tenantidtoapply']); $x++)
                {
                    $doc = 
                    [
                        'tenant_id' => $data['tenantidtoapply'][$x],
                        'amount_paid' => str_replace(",", "",$data['amount_paid'][$x])
                    ];

                    $this->db->WHERE('payment_advice_id', $advice->id)
                             ->WHERE('tenant_id', $data['tenantidtoapply'][$x])
                             ->UPDATE('payment_advice_soa', $doc);
                }
            ...
            if($this->db->trans_status() === FALSE)
            {
                $msg = ['message' => 'Something went wrong, failed updating payment advice. Please try again.', 'info' => 'error'];
            }
            else
            {
                $msg = ['message' => 'Updated Successfully.', 'info' => 'success'];
            }
        }
        else
        {
            $msg = ['message' => 'Something went wrong, data submitted seems to be empty. Please try again.', 'info' => 'error'];
        }
