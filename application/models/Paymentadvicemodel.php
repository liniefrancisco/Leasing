...
class PaymentAdviceModel extends CI_model
{
    ...
    public function getPaymentAdvices()
    {
        $query = $this->db->QUERY("SELECT * FROM payment_advice WHERE tenant_id = '" . $this->session->userdata('tenant_id') . "'")->RESULT_ARRAY();
        return $query;
    }

    public function getAdvice($advice_code)
    {
        $result = $this->db->SELECT('*')
                            ->FROM('payment_advice')
                            ->WHERE('advice_code', $advice_code)
                            ->GET()
                            ->ROW();
        return $result;
    }
